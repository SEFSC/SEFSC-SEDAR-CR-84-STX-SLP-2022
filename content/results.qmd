...
@tbl_parm presents the model-estimated values and standard deviations for the main active parameters. While it’s important to consider the scale of each parameter, the results suggest that some key parameters are not being estimated with high precision. In particular, the coefficients of variation for initial fishing mortality are relatively high across all models, indicating considerable uncertainty in these estimates.
...
Consistent with earlier results indicating that the models reached local minima (as shown by the positive definite Hessians), the jitter analysis also performed well across all model scenarios (@fig-jitter). Importantly, no jitter runs produced a lower likelihood than the best fit already identified for each model.

::: {#fig-jitter layout-ncol="2"}
!["test"](/Scenarios/84_stx_f3_5cm_010641_0041_a_m2/diagnostics/jitter/Total%20Likelihood.png)

!["test"](/Scenarios/plots_m2$/compare16_densities_SR_LN(R0).png)
