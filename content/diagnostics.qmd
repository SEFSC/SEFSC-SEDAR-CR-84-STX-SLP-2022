...
High correlations (\> 0.95) were observed in nearly all model configurations (@tbl-corr). A particularly important correlation was between the estimate of initial fishing mortality (Initial F) and unfished recruitment (R0). This correlation exceeded 0.99 in all models except for model v3_m2, where it was slightly lower but still moderately high at -0.90.

Similarly, all models, except model v3_m2, showed moderately high correlations (\> 0.90) between the two parameters used to define logistic selectivity: the size at peak selectivity and the width of the ascending limb. Correlations between these selectivity parameters are expected. While estimated values varied slightly among models, they produced similar size-based selectivity curves for the commercial dive fleet (@fig-sel).
...
Stock Synthesis also provides estimates and standard deviations for derived quantities such as annual and unfished spawning biomass, annual fishing mortality and depletion (fraction of unfished). @tbl-dq shows this information and it is also plotted in @fig-ssb and @fig-ssb-ratio.  The uncertainty associated with unfished recruitment (R0) was highest for model v7_m2. Conversely, the uncertainty in unfished recruitment was lowest for model v3_m2, however, model v3_m2 was also associated with the highest uncertainty for fishing mortality (@fig-f). The configurations for models v3_m2 and v7_m2 include estimation of recruitment deviations, the high uncertainty resulting from these model scenarios is expected due to the data limited nature of the data inputs.
...
All models nearly exactly fit the catch data. This was expected due to the low CV on catch and the date-limited and data-moderate configurations that do not provide the model with other information that would result in potentially differences between the estimated and observed catch data. Model with higher coefficients of variation on the catch data are compared in the sensitivity analyses.
