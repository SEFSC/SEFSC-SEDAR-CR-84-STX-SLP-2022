...
All m2 and m3 model scenarios showed moderately high correlations (\> 0.90) between the two parameters used to define the commercial fleet logistic selectivity: the size at peak selectivity and the width of the ascending limb. Correlations between these selectivity parameters is expected. While estimated values varied slightly among models, they produced similar size-based selectivity curves for the commercial fleet (@fig-m3-selectivity).
...
@fig-profile-eqcatch  shows the profile likelihood for the initial equilibrium catch for St. Croix stoplight parrotfish across model scenarios (a_m3, b_m3, v1_m3, v3_m3, and v7_m3). Except for model scenario v3_m3, the models suggest improved fit associated with smaller values of fixed initial equilibrium catch. Meanwhile, v3_m3 shows a minimum of 24 metric tons. @fig-profile-eqcatch-msyspr shows the corresponding change in the MSY SPR 40% across the range of initial equilibrium catch values explored. This suggests that given further flexibility the initial equilibrium may be estimated lower and was further examined through sensitivity runs.
